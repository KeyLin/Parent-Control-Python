...
def main():
    ...
    message = "曾亚南在" + time.strftime('%H:%M',time.localtime(time.time())) + "打开了电脑"

    sms_sender = config.get('SMS','sms_sender')
    feixin_pass = config.get('SMS','feixin_pass')
    sms_receivers = config.get('SMS','sms_receivers')
    if send_sms(sms_sender,feixin_pass,sms_receivers,message):  
    ...
    if send_mail(mail_host,mail_sender,mail_pass,mail_postfix,mail_receivers,message,message):  
